...
public class C04_ReadExcel extends TestBase {
    ...
    @Test
    public void testBenim() throws IOException {
        ...
        // en uzun ulke isminin Mikronezya Federal Devletleri oldugunu test edin
            ...
            List<String> ulkelerListesi=new ArrayList<>();
            int nereyeKadar=workbook.getSheet("Sayfa1").getLastRowNum();
            //System.out.println(nereyeKadar);//191

            for (int i = 0; i < nereyeKadar; i++) {
                ulkelerListesi.add(workbook.getSheet("Sayfa1").getRow(i).getCell(2).toString());
            }
            ulkelerListesi.remove(0);
            // ulkeler excelinde turkce ulke isimleri Senegal iceriyor mu test edin
            ...
            String enUzunUlkeIsmi="";

            for (String each:ulkelerListesi
            ) {
                if (each.length()>enUzunUlkeIsmi.length()){
                    enUzunUlkeIsmi=each;
                }
            }
            String expectedEnUzun="Mikronezya Federal Devletleri";
            String actualEnUzun=enUzunUlkeIsmi;

            Assert.assertEquals(expectedEnUzun,actualEnUzun);
